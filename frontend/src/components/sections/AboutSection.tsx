...
const AboutSection = memo(({
  ...
  const villaStats = [{
    icon: Maximize,
    value: villaInfo.area_sqm || '350',
    unit: 'm²',
    label: t('about.stats.area')
  }, {
    icon: Bed,
    value: villaInfo.bedrooms || '4',
    unit: '',
    label: t('about.stats.bedrooms')
  }, {
    icon: Bath,
    value: villaInfo.bathrooms || '5',
    unit: '',
    label: t('about.stats.bathrooms')
  }, {
    icon: Users,
    value: villaInfo.max_guests || '8',
    unit: '',
      ...
      <div data-scrollable="true" className="relative z-10 h-full flex flex-col justify-start sm:justify-center items-center px-4 sm:px-6 pt-20 sm:pt-20 pb-32 sm:pb-12 overflow-y-auto sm:overflow-hidden touch-pan-y overscroll-contain">
        <motion.div className="max-w-6xl w-full" variants={containerVariants} initial="hidden" animate={isActive ? 'visible' : 'hidden'}>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-3 mb-3 sm:mb-4">
            ...
            {/* Airbnb Rating */}
            <a href={villaInfo.airbnb_url || "https://hr.airbnb.com/rooms/1374488"} target="_blank" rel="noopener noreferrer" className="glass-card-coal rounded-xl p-3 sm:p-4 flex items-center gap-3 hover:bg-white/10 transition-colors cursor-pointer">
              ...
              <div>
                ...
                <p className="text-sandstone text-sm sm:text-base font-medium text-shadow-sm">{villaInfo.airbnb_rating || '4.93'}★ <span className="text-sandstone/70">(54)</span></p>
              </div>
            </a>
            ...
            <a href="https://share.google/KNtMhXhU9m7ieTJaM" target="_blank" rel="noopener noreferrer" className="glass-card-coal rounded-xl p-3 sm:p-4 flex items-center gap-3 hover:bg-white/10 transition-colors cursor-pointer">
              ...
              <div>
                ...
                <p className="text-sandstone text-sm sm:text-base font-medium text-shadow-sm">{villaInfo.google_rating || '4.8'}★ <span className="text-sandstone/70">(17)</span></p>
              </div>
            </a>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="glass-card-coal rounded-xl p-3 sm:p-4 mb-3 sm:mb-4">
            <div className="flex items-center gap-2 mb-3">
              <HelpCircle className="w-5 h-5 text-terracotta flex-shrink-0" />
              <h3 className="text-sandstone text-base sm:text-lg font-medium text-shadow-sm">{t('faq.title')}</h3>
            </div>
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="pool" className="border-sandstone/10">
                <AccordionTrigger className="text-sandstone text-sm sm:text-base py-2.5 hover:no-underline hover:text-terracotta [&[data-state=open]]:text-terracotta">
                  {t('faq.pool.question')}
                </AccordionTrigger>
                <AccordionContent className="text-sandstone/80 text-sm sm:text-base pb-3">
                  {t('faq.pool.answer')}
                </AccordionContent>
              </AccordionItem>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sandstone font-medium text-shadow-sm">{villaInfo.host_name || 'Vedran Grubišić'}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2">
                {/* Address */}
                <a href={villaInfo.google_maps_url || "https://www.google.com/maps/search/?api=1&query=Burjaki+36,+Trgetari+52224,+Croatia"} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 glass-card-coal px-3 py-2 rounded-full hover:bg-white/10 active:bg-white/20 transition-colors">
